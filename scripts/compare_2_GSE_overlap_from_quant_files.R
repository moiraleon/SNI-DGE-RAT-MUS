...
METADATA_FILE <- "metadata/SNI_MUS_2_FEMALE_GSE241361_VS_GSE123919_METADATA.csv"
...
expressed <- tpm_matrix > TPM_THRESHOLD

cat("Expression threshold set to TPM >", TPM_THRESHOLD, "\n")

gse_a_samples <- samples %>%
  filter(gse == GSE_A) %>%
  pull(sample_id)

gse_b_samples <- samples %>%
  filter(gse == GSE_B) %>%
  pull(sample_id)

cat(GSE_A, "samples:", length(gse_a_samples), "\n")
cat(GSE_B, "samples:", length(gse_b_samples), "\n")
...
overlap_table <- tibble(
  ...
  expressed_in_gse_a =
    rowSums(
      expressed[, gse_a_samples, drop = FALSE]
    ) > 0,

  expressed_in_gse_b =
    rowSums(
      expressed[, gse_b_samples, drop = FALSE]
    ) > 0,

  n_samples_expressed_gse_a =
    rowSums(
      expressed[, gse_a_samples, drop = FALSE]
    ),

  n_samples_expressed_gse_b =
    rowSums(
      expressed[, gse_b_samples, drop = FALSE]
    )
  ...
  rename(
    !!paste0("expressed_in_", GSE_A) := expressed_in_gse_a,
    !!paste0("expressed_in_", GSE_B) := expressed_in_gse_b,
    !!paste0("n_samples_expressed_", GSE_A) := n_samples_expressed_gse_a,
    !!paste0("n_samples_expressed_", GSE_B) := n_samples_expressed_gse_b
  ) %>%
  left_join(
    gene_symbols,
    by = "ensembl_gene_id"
  ...
  mutate(
    overlap_status = case_when(
      .data[[paste0("expressed_in_", GSE_A)]] &
        .data[[paste0("expressed_in_", GSE_B)]] ~ "shared",

      .data[[paste0("expressed_in_", GSE_A)]] &
        !.data[[paste0("expressed_in_", GSE_B)]] ~ paste0(GSE_A, "_only"),

      !.data[[paste0("expressed_in_", GSE_A)]] &
        .data[[paste0("expressed_in_", GSE_B)]] ~ paste0(GSE_B, "_only"),
